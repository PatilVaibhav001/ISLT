...
function showLandingScreen() {
    ...
    if (landingShell) landingShell.style.display = "flex";
...
async function checkSession() {
    try {
        ...
        if (data.user) {
            ...
            if (path === "/app") window.location.href = "/login";
            else if (path === "/login") showAuthScreen();
...
if ($("auth-close-btn")) {
    $("auth-close-btn").addEventListener("click", closeAuthScreen);
}
...
async function enterApp() {
    ...
    if (state.user) {
        if ($("sidebar-username")) $("sidebar-username").textContent = state.user.username;
        if ($("nav-username")) $("nav-username").textContent = state.user.username;
        await Promise.all([loadHistory(), loadFavorites(), loadSettings(), loadAchievementsCount()]);
    } else {
        if ($("sidebar-username")) $("sidebar-username").textContent = "Guest";
        if ($("nav-username")) $("nav-username").textContent = "Guest";
        renderHistory();
        renderFavorites();
        updateStats();
    }
...
function switchAuthTab(which) {
    ...
    if ($(`auth-tab-${which}`)) $(`auth-tab-${which}`).classList.add("active");
    if ($(`form-${which}`)) $(`form-${which}`).classList.add("active");
}
...
btnGif.addEventListener("click", async () => {
    ...
    try {
        ...
        gif.on("finished", (blob) => {
            ...
            gifStatus.innerHTML = `<a href="${url}" download="${filename}" class="gif-download-btn">Download GIF</a>`;
            btnGif.classList.remove("exporting");
        });

        gif.render();
    } catch (err) {
        gifStatus.textContent = "GIF export failed.";
        btnGif.classList.remove("exporting");
        console.error(err);
    }
});
...
function renderFavorites() {
    ...
    container.innerHTML = state.favorites.map((fav, index) => `
        <div class="list-item" style="display:flex; flex-direction:column; align-items:flex-start; gap:6px;">
            <div style="display:flex; justify-content:space-between; width:100%;">
                <span class="list-item-text" style="font-size:16px;">${index + 1}. ${escapeHtml(fav.text).toUpperCase()}</span>
                <div class="list-item-actions" style="display:flex; gap:8px;">
                    <button class="link-btn" onclick="loadFavorite('${fav.text.replace(/'/g, "\\'")}', '${fav.mode}')" style="display:flex; align-items:center; gap:6px; font-size:13px; padding:6px 12px; border:1px solid var(--border);"><img src="/static/images/others/sync.png" style="width:16px;height:16px;" alt="Load"> Load</button>
                    ${state.user ? `<button class="link-btn" onclick="deleteFavorite(${fav.id})" style="display:flex; align-items:center; gap:6px; font-size:13px; padding:6px 12px; border:1px solid var(--border); color:#dc2626;"><img src="/static/images/others/recycle-bin.png" style="width:16px;height:16px;" alt="Remove"> Remove</button>` : ""}
                </div>
            </div>
            <div class="list-item-meta" style="margin-left:16px;">
                <span class="badge" style="background:transparent; padding:0; color:#475569; font-weight:600;">${fav.mode === "ISL" ? "IN ISL" : "US ASL"}</span>
            </div>
        </div>
    `).join("");
}
...
function renderHistory() {
    ...
    container.innerHTML = state.history.map((item, index) => {
        ...
        return `
        <div class="list-item" style="display:flex; flex-direction:column; align-items:flex-start; gap:6px;">
            <span class="list-item-text" style="font-size:16px;">${index + 1}. ${escapeHtml(item.text).toUpperCase()}</span>
            <div class="list-item-meta" style="margin-left:16px;">
                <span class="badge" style="background:transparent; padding:0; color:#475569; font-weight:600;">${item.mode === "ISL" ? "IN ISL" : "US ASL"}</span>
                <span class="badge" style="background:transparent; padding:0; margin-left:16px;">${timeLabel}</span>
            </div>
        </div>
    `;
    }).join("");
...
function renderAchievements() {
    ...
    const iconMap = {
        first_translation: "ac1.png",
        ten_translations: "ac2.png",
        fifty_translations: "ac3.png",
        first_favorite: "ac4.png",
        both_modes: "ac5.png",
        quiz_complete: "ac6.png",
        quiz_perfect: "ac7.png",
        daily_goal: "ac8.png"
    };

    container.innerHTML = state.achievements.map(a => {
        const iconSrc = a.earned ? `/static/images/others/${iconMap[a.key]}` : `/static/images/others/unlock.png`;
        return `
        <div class="achievement-card ${a.earned ? "earned" : ""}">
            <div class="achievement-icon"><img src="${iconSrc}" style="width:52px;height:52px;" alt="${a.label}"></div>
            <div>
                <div class="achievement-label" style="font-size:18px;">${a.label}</div>
                <div class="achievement-desc" style="font-size:14px;">${a.desc}</div>
...
$("btn-save-settings").addEventListener("click", async () => {
    ...
    try {
        ...
        await res.json();
        $("settings-saved-msg").textContent = "Settings saved successfully";
        setTimeout(() => { $("settings-saved-msg").textContent = ""; }, 2000);
    } catch (err) {
        $("settings-saved-msg").textContent = "Could not save settings";
        $("settings-saved-msg").style.color = "#dc2626";
    }
});
